...
def get_taxonomy_instruction(rua):
    if rua == "Remember":
        return "Formattez la questions sous forme de fill-in-the-blanks, pour tester la mémoire directe de l'étudiant. Masquez un seul mot: un concept clef, central à l'idée."
    elif rua == "Understand":
...
def questionGenerator(prompt, job, difficulty):
    ...
    system_template = f"""
    En tant qu'expert en coaching, créez une question basée sur "{prompt}" pour un étudiant visant à {job_description}. {taxonomy_instruction} Difficulté: {difficulty}.
    """
