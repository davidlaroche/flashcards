...
def getFeedback(front, user_input, back):
    ...
    - Si la réponse fournie par l'étudiant est correcte, répondez 'Bravo! Réponse correcte.' et ignorez les étapes suivantes.
    - Si la réponse fournie par l'étudiant est partiellement correcte, identifiez les éléments manquants ou hors-sujet dans la réponse.
    Structurez la réponse avec des points-clefs si nécessaire. 
    La réponse ne doit pas dépasser 75 mots. 
    Restez toujours dans une démarche bienveillante et pédagogue.
    """
    system_message_prompt = SystemMessagePromptTemplate.from_template(system_template)
...
def getExample(front, prompt, example_situation, back):
    chat = ChatOpenAI(
        model="gpt-4",
        temperature=0.2
    )
    system_template = """Vous êtes un expert en coaching et en enseignement du développement personnel, et devez illustrer d'un exemple la réponse '{back}' à la question: '{front}'. 
    Vous devez vous servir de '{prompt}'. Créez un scénario fictif. La réponse ne doit pas dépasser 90 mots. Vous pouvez la formuler en liste à puces.
    Restez toujours dans une démarche bienveillante et pédagogue.
    """
    system_message_prompt = SystemMessagePromptTemplate.from_template(system_template)
    human_template = """Illustrer la réponse '{back}' à la question {front}. Personnalisez la réponse à l'étudiant: '{example_situation}'"""
    human_message_prompt = HumanMessagePromptTemplate.from_template(human_template)
    chat_prompt = ChatPromptTemplate.from_messages(
        [system_message_prompt, human_message_prompt]
    )
    chain = LLMChain(llm=chat, prompt=chat_prompt)
    result = chain.run(front=front, prompt=prompt, example_situation=example_situation, back=back)
    return result
...
def display_back(front_content, back_content, user_input):
    ...
    with st.container():
        ...
        with col4:
            st.button("✅ Facile ✅")
        with col5:
            if st.button("Obtenir du feedback"):
    ...
    with st.container():
        ...
        with colA:
            example_situation = st.text_input("Illustrez l'exemple")
        with colB:
            if st.button("Obtenir un exemple"):
                example = getExample(st.session_state.front_content, prompt, example_situation, st.session_state.back_content)
                example_placeholder.markdown(f"<div class='card'>{example}</div>", unsafe_allow_html=True)
...
personal = st.sidebar.text_input("Qu'est ce qui vous motive à devenir coach?")
